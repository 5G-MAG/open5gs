...
{
    OpenAPI_external_mbs_service_area_t *external_mbs_service_area_local_var = ogs_malloc(sizeof(OpenAPI_external_mbs_service_area_t));
    ogs_assert(external_mbs_service_area_local_var);

    external_mbs_service_area_local_var->geographic_area_list = geographic_area_list;
    external_mbs_service_area_local_var->civic_address_list = civic_address_list;

    return external_mbs_service_area_local_var;
}
...
void OpenAPI_external_mbs_service_area_free(OpenAPI_external_mbs_service_area_t *external_mbs_service_area)
{
    ...
    if (NULL == external_mbs_service_area) {
        return;
    }
    if (external_mbs_service_area->geographic_area_list) {
        OpenAPI_list_for_each(external_mbs_service_area->geographic_area_list, node) {
            OpenAPI_geographic_area_free(node->data);
        }
        OpenAPI_list_free(external_mbs_service_area->geographic_area_list);
        external_mbs_service_area->geographic_area_list = NULL;
    }
    if (external_mbs_service_area->civic_address_list) {
        OpenAPI_list_for_each(external_mbs_service_area->civic_address_list, node) {
            OpenAPI_civic_address_free(node->data);
        }
        OpenAPI_list_free(external_mbs_service_area->civic_address_list);
        external_mbs_service_area->civic_address_list = NULL;
    }
    ogs_free(external_mbs_service_area);
}
...
cJSON *OpenAPI_external_mbs_service_area_convertToJSON(OpenAPI_external_mbs_service_area_t *external_mbs_service_area)
{
    ...
    if (external_mbs_service_area == NULL) {
        ogs_error("OpenAPI_external_mbs_service_area_convertToJSON() failed [ExternalMbsServiceArea]");
        return NULL;
    }
    ...
    if (geographic_area_listList == NULL) {
        ogs_error("OpenAPI_external_mbs_service_area_convertToJSON() failed [geographic_area_list]");
        goto end;
    }
    OpenAPI_list_for_each(external_mbs_service_area->geographic_area_list, node) {
    ...
    if (civic_address_listList == NULL) {
        ogs_error("OpenAPI_external_mbs_service_area_convertToJSON() failed [civic_address_list]");
        goto end;
    }
    OpenAPI_list_for_each(external_mbs_service_area->civic_address_list, node) {
...
{
    OpenAPI_external_mbs_service_area_t *external_mbs_service_area_local_var = NULL;
    OpenAPI_lnode_t *node = NULL;
    ...
    geographic_area_list = cJSON_GetObjectItemCaseSensitive(external_mbs_service_areaJSON, "geographicAreaList");
    if (geographic_area_list) {
        cJSON *geographic_area_list_local = NULL;
        if (!cJSON_IsArray(geographic_area_list)) {
            ogs_error("OpenAPI_external_mbs_service_area_parseFromJSON() failed [geographic_area_list]");
            goto end;
        }
    ...
    civic_address_list = cJSON_GetObjectItemCaseSensitive(external_mbs_service_areaJSON, "civicAddressList");
    if (civic_address_list) {
        cJSON *civic_address_list_local = NULL;
        if (!cJSON_IsArray(civic_address_list)) {
            ogs_error("OpenAPI_external_mbs_service_area_parseFromJSON() failed [civic_address_list]");
            goto end;
        }
    ...
    external_mbs_service_area_local_var = OpenAPI_external_mbs_service_area_create (
...
OpenAPI_external_mbs_service_area_t *OpenAPI_external_mbs_service_area_copy(OpenAPI_external_mbs_service_area_t *dst, OpenAPI_external_mbs_service_area_t *src)
{
    ...
    ogs_assert(src);
    item = OpenAPI_external_mbs_service_area_convertToJSON(src);
    if (!item) {
        ogs_error("OpenAPI_external_mbs_service_area_convertToJSON() failed");
        return NULL;
    }
    ...
    OpenAPI_external_mbs_service_area_free(dst);
    dst = OpenAPI_external_mbs_service_area_parseFromJSON(item);
    cJSON_Delete(item);
