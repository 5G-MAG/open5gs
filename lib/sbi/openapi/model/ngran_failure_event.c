...
cJSON *OpenAPI_ngran_failure_event_convertToJSON(OpenAPI_ngran_failure_event_t *ngran_failure_event)
{
    ...
    if (ngran_failure_event->ngran_failure_indication == OpenAPI_ngran_failure_indication_NULL) {
        ogs_error("OpenAPI_ngran_failure_event_convertToJSON() failed [ngran_failure_indication]");
        return NULL;
    }
    if (cJSON_AddStringToObject(item, "ngranFailureIndication", OpenAPI_ngran_failure_indication_ToString(ngran_failure_event->ngran_failure_indication)) == NULL) {
        ogs_error("OpenAPI_ngran_failure_event_convertToJSON() failed [ngran_failure_indication]");
        goto end;
    }
...
OpenAPI_ngran_failure_event_t *OpenAPI_ngran_failure_event_parseFromJSON(cJSON *ngran_failure_eventJSON)
{
    ...
    OpenAPI_ngran_failure_indication_e ngran_failure_indicationVariable = 0;
    ...
    if (!cJSON_IsString(ngran_failure_indication)) {
        ogs_error("OpenAPI_ngran_failure_event_parseFromJSON() failed [ngran_failure_indication]");
        goto end;
    }
    ngran_failure_indicationVariable = OpenAPI_ngran_failure_indication_FromString(ngran_failure_indication->valuestring);

    ngran_failure_event_local_var = OpenAPI_ngran_failure_event_create (
        ngran_id_local_nonprim,
        ngran_failure_indicationVariable
    );
